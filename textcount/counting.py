...
def count_mfws(string: str, mfw_count: int) -> list[tuple]:
    """
    ...
    Returns:
        list[tuple]: A list of tuples with each tuple containing a word 
            and its count.
    """
    words: list[str] = word_tokenize(string)
    counts: Counter[str] = Counter(words)
    return counts.most_common(mfw_count)


def count_pos(string: str) -> POSCounts:
    """
...
def count_time_to_read(string: str, wpm: int) -> int:
    """
    ...
    """
    word_count: int = count_words(string)
